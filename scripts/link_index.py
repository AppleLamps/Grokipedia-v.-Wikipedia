...
Now also loads dates.txt for recency-based ranking.
"""
...
@lru_cache(maxsize=1)
def load_slugs_with_dates(base_dir: Optional[str] = None) -> Tuple[List[str], Dict[str, str]]:
    """Load all slugs and their lastmod dates from links directory.
    
    Returns:
        Tuple of (list of unique slugs, dict mapping slug -> lastmod date)
    """
    ...
    if not os.path.isdir(base):
        return slugs, slug_dates

    with _load_lock:
        seen = set()
        for root, dirs, files in os.walk(base):
            if "names.txt" not in files:
                continue
...
def load_slugs(base_dir: Optional[str] = None) -> List[str]:
    ...
    """
    slugs, _ = load_slugs_with_dates(base_dir)
    return slugs
